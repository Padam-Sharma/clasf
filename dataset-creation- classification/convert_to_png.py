...
def convert_png_process_folder(config, folder_type):
    logger.info(f'Converting tifs to png {folder_type}')
    clipped_tifs_dir = f'{config["PATH"]["output"]}/{folder_type}/tifs_clipped'
    png_dir = f'{config["PATH"]["output"]}/{folder_type}/png_clipped'
...
def convert_png_main(config, tmp_logger):
    ...
    for folder_type in folder_types:
        thread = threading.Thread(target=convert_png_process_folder, args=(config, folder_type,))
