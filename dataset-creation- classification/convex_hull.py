...
def convex_hull_process_folder(config, folder_type):
    logger.info(f'Clipping tifs for {folder_type}')
    input_parcel_dir = f'{config["PATH"]["input"]}/{folder_type}/annotations'
    output_parcel_dir = f'{config["PATH"]["input"]}/{folder_type}/annotations-conv_hull'
...
def convex_hull_main(config, tmp_logger):
    ...
    for folder_type in folder_types:
        thread = threading.Thread(target=convex_hull_process_folder, args=(config, folder_type,))
